...
func serveHTTPApi(req cmds.Request) (error, <-chan error) {
	...
	gatewayOpt := corehttp.GatewayOption(false, nil, http.Cookie{}, "", "", false, corehttp.WebUIPaths...)
	if unrestricted {
		gatewayOpt = corehttp.GatewayOption(false, nil, http.Cookie{}, "", "", true, "/ipfs", "/ipns")
...
func serveHTTPGateway(req cmds.Request) (error, <-chan error) {
	...
	var opts = []corehttp.ServeOption{
		...
		corehttp.GatewayOption(false, nil, http.Cookie{}, "", "", writable, "/ipfs", "/ipns"),
